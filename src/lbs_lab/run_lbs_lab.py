...
#print("目前尋找套件的路徑:", sys.path)
import torch
import imageio.v2 as imageio  # 確保安裝了 imageio
import matplotlib
matplotlib.use("Agg")
...
def generate_lbs_animation(model, betas, faces, joint_idx, end_angle_deg, out_dir):
    print(f"正在生成關節 {joint_idx} 從 0 到 {end_angle_deg}° 的旋轉動畫...")
    frames = []
    # 將角度轉為弧度
    end_angle_rad = np.deg2rad(end_angle_deg)
    angles = np.linspace(0, end_angle_rad, 20)
    
    device = model.v_template.device
    dtype = model.v_template.dtype
    _, body_pose = build_demo_pose(device, dtype)
    
    anim_dir = os.path.join(out_dir, "animation_frames")
    make_out_dir(anim_dir)
    
    for i, angle in enumerate(angles):
        current_body_pose = body_pose.clone()
        # 控制旋轉 (假設旋轉軸在 Y 軸，即索引為 +1)
        current_body_pose[0, (joint_idx - 1) * 3 + 1] = angle
        
        data = compute_manual_lbs(model, betas.to(device), torch.zeros((1,3), device=device), current_body_pose)
        
        frame_path = os.path.join(anim_dir, f"frame_{i:03d}.png")
        save_single_figure(frame_path, to_numpy(data["verts"][0]), faces, 
                           title=f"Joint {joint_idx} Rotation: {np.degrees(angle):.1f}°")
        frames.append(imageio.imread(frame_path))
    
    imageio.mimsave(os.path.join(out_dir, 'lbs_animation.gif'), frames, duration=0.1)
    print("動畫已儲存至: outputs/lbs_animation.gif")
...
# ... 原有的繪圖邏輯之後 ...
    ...
    # 執行動畫生成 (例如測試關節 18: 右肘)
    # generate_lbs_animation(model, betas, faces, joint_idx=18, out_dir=out_dir)
    # 根據輸入參數執行
    generate_lbs_animation(
        model, 
        betas, 
        faces, 
        joint_idx=args.anim_joint_id, 
        end_angle_deg=args.anim_end_angle, 
        out_dir=out_dir
    )
if __name__ == "__main__":
    ...
    parser.add_argument("--anim-joint-id", type=int, default=18, help="動畫中要旋轉的關節 ID")
    parser.add_argument("--anim-end-angle", type=float, default=60.0, help="動畫旋轉的結束角度（度）")
